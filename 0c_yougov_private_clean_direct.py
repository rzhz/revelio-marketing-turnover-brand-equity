...
private_list = pd.read_csv(os.path.join(directory, "private_brand_rcid_direct_2.csv"))
...
for j in range(0,3):
    ...
    final_df.to_csv(os.path.join(tdirectory, f"yougov_private_direct_job_history-{j}.csv"), index=False)
...
try:
    private_list = pd.read_csv(os.path.join(directory, "private_brand_rcid_direct_manual.csv"), encoding='latin1')
except UnicodeDecodeError:
    try:
        private_list = pd.read_csv(os.path.join(directory, "private_brand_rcid_direct_manual.csv"), encoding='ISO-8859-1')
    except UnicodeDecodeError:
        private_list = pd.read_csv(os.path.join(directory, "private_brand_rcid_direct_manual.csv"), encoding='cp1252')

for j in range(0,3):
    # Placeholder list to store filtered data
    final_df = pd.DataFrame()
    # Iterate through all the job history CSV files
    for i in range(600*j, 600*(j+1)):
        loop_start_time = time.time()
...
final_df = pd.DataFrame()
# Iterate through all the job history CSV files
for i in range(600*j, n_job_history):
    loop_start_time = time.time()
    file_name = os.path.join(sdirectory, f"cleaned_individual_job_positions_and_job_history_file-{i}.csv")
    if os.path.exists(file_name):
        job_history_df = pd.read_csv(file_name, low_memory=False)
